...
const ImageUpload = (props) => {
  ...
  return (
    <label
      className={`cursor-pointer flex items-center justify-center border border-dashed w-[400px] min-h-[400px] rounded-lg ${className} relative overflow-hidden group`}
    >
      {del && (
        <input
          type="file"
      ...
      {!image && progress === 0 && (
        <div className="flex flex-col items-center text-center pointer-events-none">
          ...
          />
          <p className="font-semibold color-pri">Choose photo</p>
        </div>
      )}
      {image && (
        <Fragment>
          <img src={image} className="object-cover w-full h-full" alt="" onClick={onClick} />
          {del && (
            <button
              type="button"
